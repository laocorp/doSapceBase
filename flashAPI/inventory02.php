...
try {
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && Functions::IsLoggedIn()) {
		...
		$playerData = $mysqli->query("SELECT * FROM player_accounts WHERE userId = {$player['userId']}")->fetch_assoc();
			...
			if ($_POST['action'] == 'init') {
					...
					if (count($array[0]->items) >= $max_item || count($array[count($array) - 1]->items) >= $max_item) {
						for ($i = 0; $i <= count($array) - 1; $i++) {
							if (isset($json_array['from']['droneItems'][$i])) {
								if (count($json_array['from']['droneItems']["$i"]) == $max_item) {
									...
									if (in_array($json_array['from']['droneItems']["$i"][0], $havocs) || in_array($json_array['from']['droneItems']["$i"][0], $herculess) || in_array($json_array['from']['droneItems']["$i"][0], $cyborgs)) {
						...
						foreach ($json_array['from']['droneItems'] as $key => $item) {
							foreach ($item as $slots) {
								if (in_array($slots, $havocs) || in_array($slots, $herculess) || in_array($slots, $cyborgs)) {
									$i = 0;
									if ($i < 1) {
				...
				if ($notOnlineOrOnlineAndInEquipZone) {
					...
					if ($json_array['from']['target'] == 'drone' && $json_array['to']['target'] == 'inventory') {
						...
						foreach ($json_array['from']['items'] as $key => $item) {
							if (in_array($item, $havocs) || in_array($item, $herculess) || in_array($item, $cyborgs)) {
...
{
	global $items, $drones, $bo2s, $G3N_7900, $lf3s, $lf4s, $lf4unstables, $lf4sps, $lf4hps, $lf4mds, $lf4pds, $mp1s, $lf3ns, $havocs, $herculess, $lf2s, $lf1s, $lf5s, $a01s, $a02s, $a03s, $bo1s, $bo3s, $G3N_1010, $G3N_2010, $G3N_3210, $G3N_3310, $G3N_6900, $cyborgs;
...
function CreateDrone($item_id, $fromId, $amount)
{
  ...
  for($i = $fromId; $i < $amount; $i++) {
	...
	$drone = ["I" => $i, "L" => $item_id, "LV" => $droneLevel, "HP" => $droneLevel+1,
		"EF" => $exp, "SP" => 15625, "DE" => "", "DL" => null, "SL" => 11, "repair" => 500, "currency" => "uridium"];
...
function GetDesignsLootIds()
{
	...
	if ($currentShip['baseShipId'] == 8) {
		...
		*/
		...
		/*
			Check all pusat designs
		*/
	    else if ($currentShip['baseShipId'] == 130) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 130");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
	...
	/*
		...
		*/
	...
	else if ($currentShip['baseShipId'] == 1300) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1300");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all mimesis designs
		*/
	    else if ($currentShip['baseShipId'] == 900) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 900");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all tartarus designs
		*/
	    else if ($currentShip['baseShipId'] == 1000) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1000");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
	...
	/*
			Check all berserker designs
		*/
	    else if ($currentShip['baseShipId'] == 1120) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1120");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all zephyr designs
		*/
		else if ($currentShip['baseShipId'] == 1100) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1100");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all disruptor designs
		*/
		else if ($currentShip['baseShipId'] == 1130) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1130");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all solaris designs
		*/
		else if ($currentShip['baseShipId'] == 1140) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1140");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all centurion designs
		*/
		else if ($currentShip['baseShipId'] == 1150) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1150");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all keres designs
		*/
		else if ($currentShip['baseShipId'] == 1165) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1165");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
		...
		/*
			Check all hecate designs
		*/
		else if ($currentShip['baseShipId'] == 1185) {
		$currentDesigns = [
		...
		$player = Functions::GetPlayer();
		$designs = $mysqli->query("SELECT * FROM player_designs WHERE userId = {$player['userId']} AND baseShipId = 1185");
		$vecDesign = [];
		while ($row = $designs->fetch_assoc()) {
			array_push($vecDesign, $row['name']);
		}
