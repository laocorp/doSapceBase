						...
						foreach ($json_array['from']['items'] as $key => $item) {
							...
							if ($i < $max_item) {
								if (($json_array['to']['slotset'] == 'lasers' && (in_array($item, $lf3s) || in_array($item, $lf4s) || in_array($item, $lf4sps) || in_array($item, $lf4unstables) || in_array($item, $lf4hps) || in_array($item, $lf4pds) || in_array($item, $lf4mds) || in_array($item, $mp1s) || in_array($item, $lf3ns) || in_array($item, $lf2s)|| in_array($item, $lf1s)|| in_array($item, $lf5s))) ||
									$json_array['to']['slotset'] == 'generators' && (in_array($item, $bo2s) || in_array($item, $bo3s) || in_array($item, $bo1s) || in_array($item, $a03s) || in_array($item, $a02s) || in_array($item, $a01s) || in_array($item, $G3N_1010) || in_array($item, $G3N_2010) || in_array($item, $G3N_3210) || in_array($item, $G3N_3310) || in_array($item, $G3N_6900) || in_array($item, $G3N_7900))
								) {
...
{
	global $items, $drones, $bo2s, $G3N_7900, $lf3s, $lf4s, $lf4unstables, $lf4sps, $lf4hps, $lf4pds, $lf4mds, $mp1s, $lf3ns, $havocs, $herculess, $lf2s, $lf1s, $lf5s, $a01s, $a02s, $a03s, $bo1s, $bo3s, $G3N_1010, $G3N_2010, $G3N_3210, $G3N_3310, $G3N_6900, $cyborgs, $spartans;
