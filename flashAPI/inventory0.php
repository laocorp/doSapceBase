...
try {
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && Functions::IsLoggedIn()) {
		...
		$hst2Count = json_decode($equipment['items'])->hst2Count;
		...
		CreateItem(30, 860, $hst2Count,$hst2lvl); //hst-2
				...
				if ($notOnlineOrOnlineAndInEquipZone) {
					...
					if ($json_array['from']['target'] == 'inventory' && $json_array['to']['target'] == 'ship' && substr($json_array['to']['slotset'], 0, -1) != null) {
						...
						foreach ($json_array['from']['items'] as $key => $item) {
							...
							if ($i < $max_item) {
								if (($json_array['to']['slotset'] == 'lasers' && (in_array($item, $lf3s) || in_array($item, $lf4s) || in_array($item, $lf4sps) || in_array($item, $lf4unstables) || in_array($item, $lf4hps) || in_array($item, $lf4pds) || in_array($item, $lf4mds) || in_array($item, $lf3ns) || in_array($item, $lf2s)|| in_array($item, $lf1s)|| in_array($item, $lf5s))) ||
									...
									$json_array['to']['slotset'] == 'heavy' && (in_array($item, $hst2s))
...
{
	global $items, $drones, $bo2s, $G3N_7900, $lf3s, $lf4s, $lf4unstables, $lf4sps, $lf4hps, $lf4mds, $lf4pds, $hst2s, $lf3ns, $havocs, $herculess, $lf2s, $lf1s, $lf5s, $a01s, $a02s, $a03s, $bo1s, $bo3s, $G3N_1010, $G3N_2010, $G3N_3210, $G3N_3310, $G3N_6900, $cyborgs, $spartans;
