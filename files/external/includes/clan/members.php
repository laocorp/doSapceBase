...
<title><?= SERVERNAME; ?> - Members</title>
...
<?php foreach(self::getMembersClan($clan['id']) as $dataMembers) { ?>
	<?php if ($player['userId'] !== $dataMembers['userId']) { ?>
		<option value="<?= $dataMembers['userId']; ?>"><?= $dataMembers['pilotName']; ?></option>
	<?php } ?>
<?php } ?>
</select>
<br><br>
...
<span style="display:inline-block; padding-left:20px; width:190px;"><?= $value['pilotName']; ?></span>
...
<span style="display:inline-block; text-align:center; width:50px;"><img src="<?= DOMAIN; ?>img/ranks/rank_<?= $value['rankId']; ?>.png"></span>
<span style="display:inline-block; text-align:center; width:60px;"><?= Functions::GetLevel(json_decode($value['data'], true)['experience']); ?></span>
<span style="display:inline-block; text-align:center; width:150px;"><?= number_format(json_decode($value['data'])->experience); ?></span>
...
<?php } else if ($clan['leaderId'] == $player['userId'] && $value['userId'] != $clan['leaderId']) { ?>
	<a style="cursor:pointer;" onclick="$('#member-delete').css('display', 'block'); document.getElementById('memberId').value = '<?= $value['userId']; ?>'; document.getElementById('member').innerHTML = '<?= $value['pilotName']; ?>';"><b><i class="far fa-trash-alt"></i> Delete</b></a>
...
<div class="col s12">
    ...
    ?>
<div id="application-user-<?php echo $user['userId']?>" style="width:748px; height:30px; line-height:30px; <?= $estilo; ?>">
 <div class="card white-text grey darken-3 padding-5">
<span style="display:inline-block; padding-left:20px; width:190px;"><?php echo $user['pilotName']; ?></span>
...
<span style="display:inline-block; text-align:center; width:50px;"><img src="<?php echo DOMAIN; ?>img/ranks/rank_<?php echo $user['rankId']; ?>.png"></span>
<span style="display:inline-block; text-align:right; width:400px;">
<?php if ($clan['leaderId'] == $player['userId'] && $value['userId'] != $clan['leaderId']) { ?>
 <a style="cursor:pointer;" onclick="deleteMemberClan(<?php echo $user['userId']; ?>); return;">Decline</a>
|<a style="cursor:pointer;" onclick="aceptMemberClan(<?php echo $user['userId']; ?>); return;">Accept</a>
<?php } ?>
</div>
</div>
